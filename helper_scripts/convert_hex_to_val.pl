...
sub convert_hex_to_ratio {
	my ($hex, $ratio) = @_;
	my $red = (hex(substr($hex, 0, 1).substr($hex, 1, 1))/255)*$ratio;
	my $green = (hex(substr($hex, 2, 1).substr($hex, 3, 1))/255)*$ratio;
	my $blue = (hex(substr($hex, 4, 1).substr($hex, 5, 1))/255)*$ratio;
	return (
		$red,
		$green,
		$blue
